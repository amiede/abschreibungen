...
public class VermoegensgegenstandImpl implements Vermoegensgegenstand {
	...
	public VermoegensgegenstandImpl(double anschaffungskosten, int nutzungsdauer) {
		// ToDo: Ggf. auf Plausibilit�t pr�fen und sonst Exception werfen
	...
	// Wir schreiben ab -- abh�ngig vom Abschreibungsverfahren
	// TODO: In mehrere private Methoden aufteilen:
	// https://sourcemaking.com/refactoring/smells/long-method
	// ToDo: Hier auch noch ein paar JavaDoc-Kommentare einf�gen
	public void abschreiben() {
		
		// Absetzung f�r Abnutzungen (kurz AfA) 
		// ToDo: �berpr�fen, ob ein anderer Name nicht mehr Sinn macht (verst�ndlicher ist)
		double afaSatz;
		double afaBetrag;
		double zeitwert;
		int wechseljahr = -1;
		...
		// Die beiden Verfahren sind fast gleich, unterscheiden sich jedoch durch die Verfahrens�nderung nach dem Wechseljahr
		...
		case GEOMETRISCH_DEGRESSIV_LINEAR:
			...
			// Falls wir von geometrisch-degressiv zu linear wechseln wollen, ben�tigen wir ein geeignetes Wechseljahr
			if (abschreibungsverfahren == Abschreibungsverfahren.GEOMETRISCH_DEGRESSIV_LINEAR) {
				
				wechseljahr = (int) ((nutzungsdauer + 1) - (100/afaSatz)); // gem�� "Faustregel"
				//System.out.println(wechseljahr); // DEBUG
				
			}
			
			// wir berechnen f�r die gesamte Nutzungsdauer jeweils Abschreibungssatz, Abschreibungsbetrag und den Zeitwert
			for (int jahr = 1; jahr <= nutzungsdauer; jahr++) {
				...
				if (jahr == wechseljahr) {
					...
					break; // restliche for-Schleife unten wird nicht mehr ausgef�hrt
				}
